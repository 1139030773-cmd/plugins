...
```
1. 出口检查：我是不是被父任务调用的子技能？
   查 RESUME.md task_stack → parent 不为 null → 是 → 继续

2. 读父任务信息：从 task_stack 取 parent 对应的任务
   → 拿到 🎯目标 📍进度 ⚠️下一步
...
🎯 目标：[读到的]
📍 进度：[更新，包含本次技能的教学成果]
⚠️ 下一步：[从父任务 next_step 推导]
```

**不准用"要继续吗""想记录吗"之类的自由发挥收尾。出口必须走格式。**
