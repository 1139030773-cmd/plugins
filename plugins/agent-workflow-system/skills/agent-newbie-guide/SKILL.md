...
第三步完成（方向明确）
    ↓
询问用户："要不要我同时帮你查一下相关背景资料？（竞品、技术方案、市场情况等）"
    ├─ 用户说"不需要"/"先不用" → 跳过，直接进入第四步
    └─ 用户说"好"/"查一下" → 触发调研
         ↓
    后台调用 deep-research，按 briefing 中已明确的目标和约束搜索
         ↓
    调研完成后，向用户同步（遵守 INTERACTION_BUDGET.md 场景豁免）
         ├─ 第一决策（方向级）：调研结果是否影响既定方向？
         │   ├─ "调整" → 回到第三步重新追问方向
         │   └─ "继续" → 触发第二决策
         │        └─ 第二决策（聚焦级）：规划优先关注哪个方面？
         │             └─ 用户选择 → 记录到 briefing 调研简报字段
         ↓
    进入第四步
```

**格式约束**：
- 两次决策**不同轮发出**，第二决策依赖第一决策答案
- 第二决策只能是聚焦/排序类，禁止新方向决策
- 调研简报注入 briefing artifact → 项目总控启动时自动读取

**不触发的任务类型**（直接跳过，进入第四步）：
- 学习技能 / 修问题 / 内容创作 / 混乱跑偏 / 阶段结束
...
- [ ] 最小动作：当前输出是最小启动信息量
- [ ] 回滚就绪：回滚点已记录
- [ ] 交互预算：每次只问 1 个问题？（调研同步场景豁免见 `INTERACTION_BUDGET.md`，上限 2 且串行触发）
