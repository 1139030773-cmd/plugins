...
| 7 | **版本一致性验证** | 检查 CHANGELOG、plugin.json、README 的版本号是否一致，不通过 → 回去补步骤 2 或 5 |

> **步骤 2 和 5 互相交叉引用**：改 CHANGELOG 时想着 plugin.json，改 plugin.json 时想着 CHANGELOG。两个版本号互为锚点，忘了一个会自相矛盾。

> **步骤 7 是硬卡点**：不手动对比 CHANGELOG/plugin.json/README 版本号 = 没改完。AI 不能凭"我记得改过了"跳过这步。
...
**写 memory / RESUME.md / .resume/ / CLAUDE.md 之前，先判断当前话题层面。开发层面的内容不写入工作流文件。**

违反信号：用户问"这是开发层面的改动 你是写进了工作流体系吗" = 你漏了这步。
